@echo off
REM Builds and runs tools/test-assign.cpp: the real RelayClient pulling from a real
...
set HERE=%~dp0
set OUT=%TEMP%\casparcg-test-assign
...
cl /nologo /EHsc /std:c++17 /Zc:__cplusplus /permissive- /MD /Gy /W3 ^
   /Fo"%OUT%\\" /Fe"%OUT%\test-assign.exe" ^
   "%HERE%test-assign.cpp" "%HERE%test-paths-stubs.cpp" ^
