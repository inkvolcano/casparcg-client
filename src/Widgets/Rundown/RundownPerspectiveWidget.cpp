...
RundownPerspectiveWidget::RundownPerspectiveWidget(const LibraryModel& model, QWidget* parent, const QString& color,
...
void RundownPerspectiveWidget::preview(const PreviewEvent& event)
{
    Q_UNUSED(event);
...
AbstractRundownWidget* RundownPerspectiveWidget::clone()
{
    RundownPerspectiveWidget* widget = new RundownPerspectiveWidget(this->model, this->parentWidget(), this->color,
    ...
    command->setUpperLeftX(this->command.getUpperLeftX());
    command->setUpperLeftY(this->command.getUpperLeftY());
    command->setUpperRightX(this->command.getUpperRightX());
    command->setUpperRightY(this->command.getUpperRightY());
    command->setLowerRightX(this->command.getLowerRightX());
    command->setLowerRightY(this->command.getLowerRightY());
    command->setLowerLeftX(this->command.getLowerLeftX());
    command->setLowerLeftY(this->command.getLowerLeftY());
...
void RundownPerspectiveWidget::setCompactView(bool compactView)
{
    if (compactView)
    {
...
void RundownPerspectiveWidget::readProperties(boost::property_tree::wptree& pt)
{
    if (pt.count(L"color") > 0) setColor(QString::fromStdWString(pt.get<std::wstring>(L"color")));
}

void RundownPerspectiveWidget::writeProperties(QXmlStreamWriter& writer)
{
    writer.writeTextElement("color", this->color);
}

bool RundownPerspectiveWidget::isGroup() const
{
    return false;
}

bool RundownPerspectiveWidget::isInGroup() const
{
    return this->inGroup;
}

AbstractCommand* RundownPerspectiveWidget::getCommand()
{
    return &this->command;
}
...
void RundownPerspectiveWidget::setInGroup(bool inGroup)
{
    this->inGroup = inGroup;
    this->labelGroupColor->setVisible(this->inGroup);
}

QString RundownPerspectiveWidget::getColor() const
{
    return this->color;
}

void RundownPerspectiveWidget::setColor(const QString& color)
{
    this->color = color;
    this->setStyleSheet(QString("#frameItem, #frameStatus { background-color: %1; }").arg(color));
}

void RundownPerspectiveWidget::checkEmptyDevice()
{
...
void RundownPerspectiveWidget::clearDelayedCommands()
{
    this->itemScheduler.cancel();
}

void RundownPerspectiveWidget::setUsed(bool used)
{
    if (used)
    {
...
void RundownPerspectiveWidget::executeStop()
{
    this->itemScheduler.cancel();
...
void RundownPerspectiveWidget::executePlay()
{
    const QSharedPointer<CasparDevice> device = DeviceManager::getInstance().getDeviceByName(this->model.getDeviceName());
    if (device != NULL && device->isConnected())
...
void RundownPerspectiveWidget::executePlayPreview()
{
    ...
    if (deviceModel != NULL && deviceModel->getPreviewChannel() > 0)
    {
        const QSharedPointer<CasparDevice> device = DeviceManager::getInstance().getDeviceByName(this->model.getDeviceName());
        if (device != NULL && device->isConnected())
            device->setPerspective(deviceModel->getPreviewChannel(), this->command.getVideolayer(), this->command.getUpperLeftX(),
                                   this->command.getUpperLeftY(), this->command.getUpperRightX(), this->command.getUpperRightY(),
                                   this->command.getLowerRightX(), this->command.getLowerRightY(), this->command.getLowerLeftX(),
                                   this->command.getLowerLeftY(), this->command.getTransitionDuration(), this->command.getTween(),
                                   this->command.getDefer(), this->command.getUseMipmap());
    ...
    foreach (const DeviceModel& model, DeviceManager::getInstance().getDeviceModels())
    {
        ...
        if (model.getPreviewChannel() > 0)
        {
            const QSharedPointer<CasparDevice>  deviceShadow = DeviceManager::getInstance().getDeviceByName(model.getName());
            if (deviceShadow != NULL && deviceShadow->isConnected())
                deviceShadow->setPerspective(model.getPreviewChannel(), this->command.getVideolayer(), this->command.getUpperLeftX(),
                                             this->command.getUpperLeftY(), this->command.getUpperRightX(), this->command.getUpperRightY(),
                                             this->command.getLowerRightX(), this->command.getLowerRightY(), this->command.getLowerLeftX(),
                                             this->command.getLowerLeftY(), this->command.getTransitionDuration(), this->command.getTween(),
                                             this->command.getDefer(), this->command.getUseMipmap());
...
void RundownPerspectiveWidget::executeClearVideolayer()
{
    this->itemScheduler.cancel();
...
void RundownPerspectiveWidget::executeClearChannel()
{
    this->itemScheduler.cancel();
...
void RundownPerspectiveWidget::channelChanged(int channel)
{
...
void RundownPerspectiveWidget::videolayerChanged(int videolayer)
{
...
void RundownPerspectiveWidget::delayChanged(int delay)
{
...
void RundownPerspectiveWidget::checkGpiConnection()
{
...
void RundownPerspectiveWidget::checkDeviceConnection()
{
    const QSharedPointer<CasparDevice> device = DeviceManager::getInstance().getDeviceByName(this->model.getDeviceName());
    if (device == NULL)
...
void RundownPerspectiveWidget::configureOscSubscriptions()
{
...
void RundownPerspectiveWidget::allowGpiChanged(bool allowGpi)
{
    Q_UNUSED(allowGpi);
...
void RundownPerspectiveWidget::gpiConnectionStateChanged(bool connected, GpiDevice* device)
{
    Q_UNUSED(connected);
    Q_UNUSED(device);
...
void RundownPerspectiveWidget::deviceConnectionStateChanged(CasparDevice& device)
{
    Q_UNUSED(device);
...
void RundownPerspectiveWidget::remoteTriggerIdChanged(const QString& remoteTriggerId)
{
...
void RundownPerspectiveWidget::deviceAdded(CasparDevice& device)
{
...
void RundownPerspectiveWidget::stopControlSubscriptionReceived(const QString& predicate, const QList<QVariant>& arguments)
{
    Q_UNUSED(predicate);
...
void RundownPerspectiveWidget::playControlSubscriptionReceived(const QString& predicate, const QList<QVariant>& arguments)
{
    Q_UNUSED(predicate);
...
void RundownPerspectiveWidget::playNowControlSubscriptionReceived(const QString& predicate, const QList<QVariant>& arguments)
{
    Q_UNUSED(predicate);
...
void RundownPerspectiveWidget::nextControlSubscriptionReceived(const QString& predicate, const QList<QVariant>& arguments)
{
    Q_UNUSED(predicate);
...
void RundownPerspectiveWidget::updateControlSubscriptionReceived(const QString& predicate, const QList<QVariant>& arguments)
{
    Q_UNUSED(predicate);
...
void RundownPerspectiveWidget::clearControlSubscriptionReceived(const QString& predicate, const QList<QVariant>& arguments)
{
    Q_UNUSED(predicate);
...
void RundownPerspectiveWidget::clearVideolayerControlSubscriptionReceived(const QString& predicate, const QList<QVariant>& arguments)
{
    Q_UNUSED(predicate);
...
void RundownPerspectiveWidget::clearChannelControlSubscriptionReceived(const QString& predicate, const QList<QVariant>& arguments)
{
    Q_UNUSED(predicate);
...
void RundownPerspectiveWidget::setRundownDisabled(bool disabled)
{
