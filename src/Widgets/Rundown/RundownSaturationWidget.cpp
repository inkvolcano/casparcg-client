...
void RundownSaturationWidget::executePlayPreview()
{
    ...
    if (deviceModel != NULL && deviceModel->getPreviewChannel() > 0)
    {
        const QSharedPointer<CasparDevice> device = DeviceManager::getInstance().getDeviceByName(this->model.getDeviceName());
        if (device != NULL && device->isConnected())
            device->setSaturation(deviceModel->getPreviewChannel(), this->command.getVideolayer(), this->command.getSaturation(),
                                  this->command.getTransitionDuration(), this->command.getTween(), this->command.getDefer());
    }
    ...
    foreach (const DeviceModel& model, DeviceManager::getInstance().getDeviceModels())
    {
        ...
        if (model.getPreviewChannel() > 0)
        {
            const QSharedPointer<CasparDevice>  deviceShadow = DeviceManager::getInstance().getDeviceByName(model.getName());
            if (deviceShadow != NULL && deviceShadow->isConnected())
                deviceShadow->setSaturation(model.getPreviewChannel(), this->command.getVideolayer(), this->command.getSaturation(),
                                            this->command.getTransitionDuration(), this->command.getTween(), this->command.getDefer());
        }
    }
}
...
void RundownSaturationWidget::checkGpiConnection()
{
...
void RundownSaturationWidget::checkDeviceConnection()
{
    const QSharedPointer<CasparDevice> device = DeviceManager::getInstance().getDeviceByName(this->model.getDeviceName());
    if (device == NULL)
...
void RundownSaturationWidget::remoteTriggerIdChanged(const QString& remoteTriggerId)
{
...
void RundownSaturationWidget::deviceAdded(CasparDevice& device)
{
...
void RundownSaturationWidget::playNowControlSubscriptionReceived(const QString& predicate, const QList<QVariant>& arguments)
{
    Q_UNUSED(predicate);
...
void RundownSaturationWidget::setRundownDisabled(bool disabled)
{
