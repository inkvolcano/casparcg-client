...
RundownCommandGatewayWidget::RundownCommandGatewayWidget(const LibraryModel& model, QWidget* parent, const QString& color,
      ...
      active(active), inGroup(inGroup), compactView(compactView), color(color), model(model)
{
    ...
    QObject::connect(&EventManager::getInstance(), &EventManager::gatewayExitsChanged, this, &RundownCommandGatewayWidget::gatewayExitsChanged);

    QObject::connect(&this->command, &GatewayCommand::exitLabelChanged, this, &RundownCommandGatewayWidget::updateVisuals);
    QObject::connect(&this->command, &GatewayCommand::selectedExitLabelChanged, this, &RundownCommandGatewayWidget::updateVisuals);

    this->conditionTimer = new QTimer(this);
    this->conditionTimer->setInterval(30000);
    QObject::connect(this->conditionTimer, &QTimer::timeout, this, &RundownCommandGatewayWidget::rebuildExitButtons);

    updateVisuals();
}

void RundownCommandGatewayWidget::updateVisuals()
{
    ...
    RundownWidgetHelper::setupGatewayBadge(this->labelColor, this->command.getIsExit(), "rgba(180, 100, 40, 220)", "command");
    rebuildExitButtons();
}

void RundownCommandGatewayWidget::setTreeItem(QTreeWidgetItem* item)
{
    this->treeItem = item;
    rebuildExitButtons();
}

void RundownCommandGatewayWidget::gatewayExitsChanged(const QString& gatewayId)
{
    if (this->command.getGatewayId() == gatewayId && !this->command.getIsExit())
        rebuildExitButtons();
}

void RundownCommandGatewayWidget::rebuildExitButtons()
{
    ...
    static const QString SELECTED_STYLE = "QPushButton { background-color: rgba(180, 100, 40, 200); border: 1px solid rgba(255,255,255,40); color: white; font-size: 11px; text-align: left; padding-left: 6px; }";
    ...
    static const QString RETURN_STYLE = "QPushButton { background-color: rgba(120, 70, 30, 200); border: 1px solid rgba(255,255,255,30); color: rgba(200,200,200,220); font-size: 11px; text-align: left; padding-left: 6px; }";
...
void RundownCommandGatewayWidget::labelChanged(const LabelChangedEvent& event)
{
    if (!this->selected)
        return;
...
AbstractRundownWidget* RundownCommandGatewayWidget::clone()
{
    RundownCommandGatewayWidget* widget = new RundownCommandGatewayWidget(this->model, this->parentWidget(), this->color,
...
void RundownCommandGatewayWidget::readProperties(boost::property_tree::wptree& pt)
{
...
void RundownCommandGatewayWidget::writeProperties(QXmlStreamWriter& writer)
{
    writer.writeTextElement("color", this->color);
}

void RundownCommandGatewayWidget::setCompactView(bool compactView)
{
    if (compactView)
    {
...
bool RundownCommandGatewayWidget::isGroup() const
{
    return false;
}

bool RundownCommandGatewayWidget::isInGroup() const
{
    return this->inGroup;
}

AbstractCommand* RundownCommandGatewayWidget::getCommand()
{
    return &this->command;
}

LibraryModel* RundownCommandGatewayWidget::getLibraryModel()
{
    return &this->model;
}

QString RundownCommandGatewayWidget::getColor() const
{
    return this->color;
}

void RundownCommandGatewayWidget::setColor(const QString& color)
{
    this->color = color;
...
void RundownCommandGatewayWidget::setActive(bool active)
{
    this->active = active;
...
void RundownCommandGatewayWidget::setInGroup(bool inGroup)
{
