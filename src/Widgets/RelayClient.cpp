...
// Set once at startup by the application, which is the only place that can see
// the generated version header. Empty in a test harness, and empty reports
// nothing rather than reporting a wrong answer.
QString RelayClient::clientVersion;
QString RelayClient::clientBuild;

void RelayClient::setClientVersion(const QString& version, const QString& build)
{
    clientVersion = version.trimmed();
    clientBuild = build.trimmed();
}
...
void RelayClient::sendCheckIn()
{
    ...
    // Which build this venue is actually running.
    //
    // Nothing has ever reported this, so "what is the estate on" has only been
    // answerable by visiting machines. It rides along here rather than becoming a
    // second request because it changes about as often as the pack versions beside
    // it, and the report is already the thing that says what a venue holds.
    //
    // Told to this class rather than read from Version.h, because that header is
    // generated by CMake and the standalone test harnesses compile this file with
    // no build directory to find it in. A client that was never told reports
    // neither field, which reads the same as a client too old to know.
    if (!clientVersion.isEmpty())
        body.insert("version", clientVersion);
    if (!clientBuild.isEmpty())
        body.insert("build", clientBuild);
